...
/* ============================================================
   ...
   Two pink tiers, dripping cream, sprinkles, pink heart topper,
   star/heart side decals, candles. Same engine protocol as
   Cake3D (candles / blowing props).
   ============================================================ */
...
/* striped body per [c1,c2] pair (app-provided colors) */
const texCache = {};
...
function defaultCandleTexture(){
  ...
  g.fillStyle='#f27ba8';                                            // pink stripes
...
const cakeCapMat  = new THREE.MeshStandardMaterial({ color:0xf2a9c6, roughness:0.7 });   // pink caps
const ribbonMat   = new THREE.MeshStandardMaterial({ color:0xee6fa2, roughness:0.45 });  // deep pink ribbon
const plateMat    = new THREE.MeshStandardMaterial({ color:0xf27ba8, roughness:0.5 });   // pink plate
const plateInMat  = new THREE.MeshStandardMaterial({ color:0xffedf4, roughness:0.6 });   // light pink inner plate
...
export default function PeachCake3D({ candles, blowing }){
  ...
  useEffect(() => {
    ...
    /* ---- pink heart topper ---- */
    const heart=new THREE.Mesh(heartGeo(0.85,0.18),
      new THREE.MeshStandardMaterial({ color:0xf27ba8, roughness:0.45 }));
    // Sunk into the icing dome so it looks planted, not floating:
    // the heart's bottom ~1/3 is embedded in the cake top
    heart.position.set(0, t2TopY + 0.24, 0.05);
    heart.rotation.x = -0.10;                 // tiny back-tilt, like a topper stuck in
    ...
    const SLOTS=11, STEP=(Math.PI*2)/SLOTS;          // 9 stars + 2 hearts
    ...
    function addCandle(slot){
      ...
      const grp=new THREE.Group(); grp.position.set(x, t2TopY-0.05, z);
    ...
    const sprinkles=new THREE.InstancedMesh(
      ...
      new THREE.MeshStandardMaterial({ color:0xffffff, roughness:0.35 }), 280);
    ...
    function onDome(s, rMin, rMax, n){
      for(let k=0;k<n && si<280;k++,si++){
        const a=rndK()*Math.PI*2;
        const r=Math.sqrt(rMin*rMin+(rMax*rMax-rMin*rMin)*rndK());
        dummy.position.set(Math.cos(a)*r, domeY(s,r)+0.015, Math.sin(a)*r);
    ...
    onDome(SURF.t2, 0.42, 1.18, 90);
    onDome(SURF.t1, 1.58, 2.20, 95);
